...
def _update_engagement_db_message_from_coda_message(engagement_db, engagement_db_message, coda_message, coda_config,
    ...
    if ws_code is not None:
        try:
            ...
            if coda_config.ws_correct_dataset_code_scheme.get_code_with_match_value(ws_code.string_value):
        ...
        assert correct_dataset not in engagement_db_message.previous_datasets, \
            ...
            f"Coda message id {coda_message.message_id}"\
            f"WS code string value {ws_code.string_value}"\
            f"Correct dataset {correct_dataset}"
